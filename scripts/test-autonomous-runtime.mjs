...
assert.match(index, /Adaptive strategy 65 · expanded protected search · build 158/);
...
assert.match(index, /const SHOCK_FADE_PROBATION_MIN_JACKKNIFE_MEAN=0;/);
assert.match(index, /const SHOCK_FADE_PROBATION_POSITION_PCT=0\.0025;/);
assert.match(index, /const SHOCK_FADE_PROBATION_TOTAL_CAPITAL_PCT=0\.01;/);
assert.match(index, /fiveEventsCannotBypassCrossEraAudit:/);
assert.match(index, /singleLargeDownsideBlocksEarlyProbation:/);
assert.match(index, /oneOutlierCannotCreateProbation:/);
assert.match(index, /disabledProbationSurvivesCloudCompaction:/);
assert.match(index, /disabledProbationCannotAddCapital:/);
assert.match(index, /emptyBundleMakerEvidenceStartsNeutral:/);
...
assert.match(index, /const DIRECTIONAL_ADAPTIVE_OFFLINE_APPROVED = false;/);
assert.match(index, /directional_capital_approved:DIRECTIONAL_ADAPTIVE_OFFLINE_APPROVED/);
...
assert.match(index, /strongerIndependentEvidenceScalesCapital:/);
assert.match(index, /weakerEvidenceAutomaticallyDemotesCapital:/);
assert.match(index, /confidenceLossCancelsPendingCapitalBeforeFill:/);
assert.match(index, /evidenceScaledAllocationSurvivesOfflineCompaction:/);
assert.match(index, /evidenceScaledAllocationSurvivesSharedCompaction:/);
assert.match(index, /makerOutcomeUsesProtectedReserveDenominator:/);
assert.match(index, /makerOutcomeAuditSurvivesSharedCompaction:/);
assert.match(index, /priorPolicyOutcomesCannotBlockCurrentResearch:/);
assert.match(index, /currentNoFillUsesTwoHourRetry:/);
assert.match(index, /verifiedFillRetainsTwentyFourHourCooldown:/);
assert.match(index, /protectedDepthSearchCoversFiveHundredStructures:/);
...
assert.match(api, /shock_lane_position_pct: s\.shock_lane_position_pct, shock_lane_total_capital_pct: s\.shock_lane_total_capital_pct/);
...
assert.match(workflow, /EXPECTED_BUILD: "158"/);
...
assert.match(routingReleaseWorkflow, /BUNDLE_DEPTH_CANDIDATE_LIMIT=500/);
...
assert.match(index, /const BUNDLE_MAKER_STRATEGY_VERSION = 6;/);
...
assert.match(index, /legacyBundleMakerPolicyCannotContaminateCurrentEvidence:/);
assert.match(index, /const BUNDLE_DEPTH_CANDIDATE_LIMIT=500;/);
...
assert.match(index, /const BUNDLE_EXIT_CAPTURE_EXTENDED=0\.20;/);
assert.match(index, /const BUNDLE_EXIT_MIN_REALIZED_PROFIT=0\.50;/);
assert.match(index, /const BUNDLE_MAKER_PROMOTION_EVENTS=30;/);
assert.match(index, /const BUNDLE_MAKER_PROMOTION_LOCKS=5;/);
assert.match(index, /const BUNDLE_MAKER_MAX_ACTIVE=4;/);
assert.match(index, /const BUNDLE_MAKER_RESEARCH_CANDIDATE_LIMIT=20;/);
assert.match(index, /const BUNDLE_MAKER_MIN_SPREAD_POSITION=0\.05;/);
assert.match(index, /const BUNDLE_MAKER_EXTENDED_HORIZON_HOURS=3;/);
assert.match(index, /function bundleMakerEvidenceHorizonHours\(assist,\{capitalEnabled=false\}=\{\}\)/);
assert.match(index, /highQualityShadowGetsLongerEvidenceWindow:/);
assert.match(index, /if\(!baseline\|\|!baseline\.protective_eligible\)return baseline;/);
assert.match(index, /function manageBundleMakerQuotes\(st,\{executeTrades=false,touchHistory=\{\},booksOverride=null,reportedFeesOverride=null,refreshCandidates=\[\]\}=\{\}\)/);
assert.match(index, /function refreshBundleMakerQuote\(quote,candidate\)/);
assert.match(index, /function bundleMakerPostTouchQuote\(quote,books,reportedFees\)/);
assert.match(index, /offlineBundleMakerCannotInventFill:/);
assert.match(index, /longDatedBundleRecyclesPositiveProfitSooner:/);
assert.match(index, /atomicallyRecyclesEightySixPercentProfit:/);
assert.match(index, /const MAKER_RESEARCH_RANKING_MIN_EVENTS=3;/);
assert.match(index, /const MAKER_EVENT_RESEARCH_COOLDOWN_HOURS=24;/);
assert.match(index, /function recentMakerResearchEventKeys\(st,now=Date\.now\(\)\)/);
assert.match(index, /function retireCooldownShadowQuotes\(st,completedKeys=recentMakerCompletedEventKeys\(st\)\)/);
assert.match(index, /earlyIndependentLossesReduceResearchPriority:/);
assert.match(index, /recentMakerEventsEnterSharedCooldown:/);
assert.match(index, /inheritedDuplicateShadowQuotesRetireWithoutPnl:/);
...
for (const [index, portfolio] of Object.values(transportState.agents).entries()) {
  ...
  portfolio.bundle_maker_quotes = Array.from({ length: 3 }, (_, row) => ({ id: `bundle-quote-${index}-${row}`, touch_token_id: `token-${row}` }));
  portfolio.bundle_maker_outcomes = Array.from({ length: 45 }, (_, row) => ({ quote_id: `bundle-${index}-${row}`,
    event_key: `bundle-event-${row}`, status: "hedged-lock", pnl: 1, return_on_reserved: 0.02,
    bundle_maker_strategy_version: 1, build_version: build }));
}
...
assert.equal(transportedState.agents.value.bundle_maker_quotes.length, 2);
assert.equal(transportedState.agents.value.bundle_maker_quotes.at(-1).id, "bundle-quote-0-2");
assert.equal(transportedState.agents.value.bundle_maker_outcomes.length, 30);
assert.equal(transportedState.agents.value.bundle_maker_outcomes.at(-1).event_key, "bundle-event-44");
assert.equal(transportedState.agents.value.bundle_maker_outcomes.at(-1).return_on_reserved, 0.02);
