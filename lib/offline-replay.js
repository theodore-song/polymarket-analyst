...
export function tradeFor(row, rule, horizon, costCents = 2) {
  ...
  return {
    ...
    netReturn: Math.max(-1, Math.min(2, (exit - entry - cost) / entry)),
  };
}
...
export function applyTieredRiskPolicy(rows, trades, horizon, costCents = 2, options = {}) {
  const stopLoss = options.stopLoss ?? 0.18;
  const gainTiers = options.gainTiers || [0.15, 0.30, 0.50];
  const gainFraction = options.gainFraction || 0.25;
  const byMarket = new Map();
  for (const row of rows) {
    const points = byMarket.get(row.marketId) || [];
    points.push(row);byMarket.set(row.marketId, points);
  }
  for (const points of byMarket.values()) points.sort((a, b) => a.observedAt - b.observedAt);
  const cost = Math.max(0, costCents) / 100;
  const netAt = (exit, entry) => Math.max(-1, Math.min(2, (exit - entry - cost) / entry));
  return trades.map((trade) => {
    const entry = Number(trade.entryPrice);
    if (!(entry > 0)) return trade;
    const end = trade.observedAt + horizon * HOUR;
    const path = (byMarket.get(trade.marketId) || []).filter((row) =>
      row.observedAt > trade.observedAt && row.observedAt <= end);
    let remaining = 1;
    let realized = 0;
    let gainIndex = 0;
    let exitReason = "fixed-horizon";
    for (const point of path) {
      const sidePrice = trade.side === "YES" ? Number(point.price) : 1 - Number(point.price);
      if (!Number.isFinite(sidePrice)) continue;
      const rawReturn = sidePrice / entry - 1;
      if (rawReturn <= -stopLoss) {
        realized += remaining * netAt(sidePrice, entry);
        remaining = 0;exitReason = "stop-loss";break;
      }
      while (gainIndex < gainTiers.length && rawReturn >= gainTiers[gainIndex] && remaining > 0) {
        const fraction = Math.min(gainFraction, remaining);
        realized += fraction * netAt(sidePrice, entry);
        remaining -= fraction;gainIndex++;exitReason = "staged-gain";
      }
    }
    if (remaining > 0) realized += remaining * trade.netReturn;
    return { ...trade, netReturn: Math.max(-1, Math.min(2, realized)), riskExit: exitReason,
      gainTiersFilled: gainIndex, remainingAtHorizon: +remaining.toFixed(4) };
  });
}
