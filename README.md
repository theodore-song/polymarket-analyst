...
that as a separate immediate path. The scanner reserves 40 of its 300 depth checks
...
Build 141 expands the independently prioritized protected-structure depth shortlist
from 200 to 300. A live audit of the same 1,000-event universe found one qualifying
maker observation in the first 200 structures and a second independent qualifying
observation in structures 201-300. The ordinary directional universe remains capped
at the 500 most-active eligible markets; this change spends additional exact depth
...
immediately with at least $0.25 total profit and the duration-adjusted hurdle still
...
Build 147 adds a separate six-hour evidence window for reward-qualified paired maker
observations after the current 30-day stress audit found that six hours was the only tested
short horizon with a path-stable candidate at a 75% reward haircut. Reward estimates,
quote duration, and reward share now survive the bounded cross-device snapshot, allowing the
...
Build 156 corrects the protected-maker learner's return denominator. Every lock, unwind, loss, and no-fill now uses the pre-touch complete-bundle reserve for `return_on_reserved`, while actual touched or hedged capital remains separately recorded as `deployed_capital`. Strategy-versioned evidence prevents older touched-leg-denominator losses from contaminating the corrected learner, and shared compaction retains the reserve, touch source, and hedge status needed for cross-device audits.
Build 157 makes protected-maker research cooldowns policy- and outcome-aware. Prior-policy and administrative observations no longer suppress a current learner, current-policy no-fills retry after two hours, and locks or adverse fills retain the 24-hour event cooldown. The learner still deduplicates every retry to the latest observation for one event, so research resumes sooner without counting correlated attempts as independent evidence.
Build 158 expands the live protected depth shortlist from 300 to 500 structures per cycle while retaining the top-500 active-market public universe and every existing economic and risk gate. This uses the remaining time in the serialized five-minute runtime slot to search more complete sets, conversions, exclusivity pairs, dominance pairs, and binary complements for genuinely executable protected opportunities.
...
capital and the completed bundle's modeled lock profit must cover that unwind loss
at least one-for-one. Wider profitable spreads can therefore receive a more
competitive quote without increasing position size or relaxing the immediate
hedge-or-unwind requirement. Its $0.25 absolute profit floor is calibrated to the
bounded maker pilot instead of borrowing the ordinary all-taker route's $0.50 floor;
minimum return and duration-adjusted efficiency remain mandatory. The live audit
records every maker rejection reason. It also requires at least 12 hours before the
scheduled resolution, blocking ended and near-settlement books after a live soccer
shadow exposed a 12.1% adverse-selection unwind. Active quotes from older policy
versions retire before touch checks and do not create cooldowns. Quote policy version
4 has its own independent evidence; older outcomes remain stored but cannot promote
the new rule.
