...
Vercel Blob. Build 59 also installs an offline app shell and caches timestamped
market snapshots. During an outage, cycles continue locally; cached entries are
allowed for 90 minutes, older snapshots become mark-only, and all cached data
expires after 24 hours.

Build 59 ranks the competition by each agent's return since Strategy 50 began.
...
Build 59 enforces the documented offline boundary end to end. Cached snapshots
...
Build 59 independently refreshes markets for matured pending signals that have
...
Build 59 also allocates the 300 pending observation slots by evidence coverage.
